...
void Autonome::departMilieu(BaseRoulante* base, Pince* pince)
{
	...
	default:
		 //On re-appelle ejecter cube en simulant un bouton relach� pour que la pince s'arrete
		break;
	}
	...
	//Si un PID est consider� comme fini alors on passe � l'etat suivant et on reset capteurs et variables
...
void Autonome::memeCote(BaseRoulante* base, Pince* pince)
{
	...
	case 5:
		//Qu'est ce que tu imagine qui ce passe  � cette etape?
		std::cout <<"ejection"<<std::endl;
		break;
	...
	pince->ejecterCube(false); //On appelle ejecter cube en simulant un bouton relach� pour que la pince s'arrete
...
void Autonome::coteOppose(BaseRoulante* base, Pince* pince)
{
	...
	case 6:
		...
		pince->positionVerrin(true); //On simule un bouton appuy� et on passe au suivant
		etat++;
		break;

	case 7:
		pince->goToEchangeur(false); //On simule un bouton appuy� et on passe au suivant
		break;
